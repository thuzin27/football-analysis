...
/**
 * DataKick Predictor (DKP) — algoritmo proprietário de predição.
 *
 * Pipeline de 3 camadas:
 *
 *   1. Exponential Decay Weighting
 *      Cada jogo recebe peso w_i = e^(-λ·(n-1-i)), onde i=0 é o mais antigo.
 *      Jogos mais recentes têm influência maior nas probabilidades.
 *
 *   2. Markov Chain (ordem 1)
 *      Matriz de transição 3×3 (V, E, D) construída a partir da sequência
 *      histórica. Captura "momentum": após V, qual é a prob de V novamente?
 *
 *   3. Bayesian Blend
 *      Combina probabilidades históricas ponderadas (prior) com a linha
 *      Markov do estado atual (likelihood):
 *          P_final = α·P_markov + (1-α)·P_historico_ponderado
 *
 * Para gols esperados usa distribuição de Poisson implícita:
 *      λ = média ponderada de gols → P(marcar k gols) = e^(-λ)·λ^k / k!
 */
public class PredictionAlgorithm {

    // Controles do modelo
    private static final double LAMBDA_DECAY  = 0.18;  // velocidade de decaimento
    private static final double ALPHA_MARKOV  = 0.38;  // peso da cadeia de Markov
    // (1 - ALPHA_MARKOV) é o peso do prior histórico ponderado
    ...
    public static class Predicao {
        ...
        public double golsEsperados;       // λ gols marcados
        public double golsSofridosEsperados;
        public double confianca;           // 0..1, baseada em n amostras
    ...
    public static Predicao prever(List<Partida> historico) {
        ...
        // ── 3. MARKOV CHAIN — MATRIZ DE TRANSIÇÃO 3×3 ────────────────
        //   Índice: 0=V, 1=E, 2=D
